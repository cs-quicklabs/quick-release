...
const ChangeLogListItem: React.FC<{ id: string }> = ({ id }) => {
  ...
  const changeLog = useMemo<ChangeLogType | null | undefined>(
    () => map[id],
    [map, id]
  );
  ...
  return (
    <main className="max-w-3xl border-b border-gray-100">
      <div className="md:flex md:items-center md:justify-between">
        <div className="min-w-0 flex-1">
          <Link href={publicLink}>
            <h2 className="hover:underline text-2xl font-bold  text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">
              {title}
            </h2>
          </Link>
      ...
      <div className="content-container space-y-6 text-sm text-gray-800 py-12 ql-snow">
        ...
        {showReadMoreBtn && (
          <Link className="text-sm" href={publicLink}>
            {"Read More "}
