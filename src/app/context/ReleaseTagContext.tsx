...
const ReleaseTagProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  const createReleaseTag = async (data: IReleaseTag, setIsLoading: (loading: boolean) => void, isNotify = true) => {
    ...
    await requestHandler(
      async () => await createReleaseTagRequest({ ...data, organizationsId: loggedInUser?.orgs[0]?.id }),
      setIsLoading,
      (res: any) => {
  ...
  const getAllReleaseTags = async (setIsLoading: (loading: boolean) => void) => {
    await requestHandler(
      async () => await getAllReleaseTagsRequest({organizationsId: loggedInUser?.orgs[0]?.id}),
      setIsLoading,
      (res: any) => {
        const { data } = res;
  ...
  const updateReleaseTag = async (data: IReleaseTag, setIsLoading: (loading: boolean) => void) => {
    setError("");
    await requestHandler(
      async () => await updateReleaseTagRequest({ ...data, organizationsId: loggedInUser?.orgs[0]?.id }),
      setIsLoading,
      (res: any) => {
        const { data, message } = res;
        const releaseTagId = data.id!;
  ...
  const deleteReleaseTag = async (id: number, setIsLoading: (loading: boolean) => void) => {
    setError("");
    await requestHandler(
      async () => await deleteReleaseTagRequest({ id, organizationsId: loggedInUser?.orgs[0]?.id }),
      setIsLoading,
      (res: any) => {
        const { message } = res;
